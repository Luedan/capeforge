...
function TaskTableRow({ task }: { task: TaskItem }) { const done = Boolean(task.completedAt); return <tr className={cn("border-b border-line/70 transition last:border-0 hover:bg-white/[.025]", done && "bg-emerald-400/[.025]")}><td className="px-5 py-4"><TaskToggle taskId={task.id} completed={done} /></td><td className="max-w-xl px-4 py-4"><strong className={cn("block text-sm", done ? "text-muted line-through" : "text-cream")}>{task.name}</strong><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted">{task.compInstructions ?? task.description ?? "Sin descripción disponible."}</p></td><td className="px-4 py-4"><Badge>{task.category ?? "General"}</Badge>{task.subcategory && <span className="mt-1.5 block text-[10px] text-muted">{task.subcategory}</span>}</td><td className="px-4 py-4"><div className="flex flex-wrap gap-1.5">{task.compDifficulty && <Badge tone={task.compDifficulty.toLowerCase().includes("dific") ? "red" : "gold"}>{task.compDifficulty}</Badge>}{task.compTimeType && <Badge tone="purple">{task.compTimeType}</Badge>}</div></td><td className="px-5 py-4">{task.wikiUrl && <a href={task.wikiUrl} target="_blank" rel="noreferrer" className="grid size-9 place-items-center rounded-xl text-muted transition hover:bg-white/5 hover:text-gold-light" aria-label={`Abrir ${task.name} en la wiki`}><ExternalLink className="size-4" /></a>}</td></tr>; }
function TaskMobileRow({ task }: { task: TaskItem }) { const done = Boolean(task.completedAt); return <div className="p-4"><div className="flex items-start gap-3"><TaskToggle taskId={task.id} completed={done} /><div className="min-w-0 flex-1"><strong className={cn("block text-sm", done && "text-muted line-through")}>{task.name}</strong><p className="mt-1.5 text-xs leading-5 text-muted">{task.compInstructions ?? task.description}</p><div className="mt-3 flex flex-wrap gap-1.5"><Badge>{task.category ?? "General"}</Badge>{task.compDifficulty && <Badge tone="gold">{task.compDifficulty}</Badge>}{task.wikiUrl && <a href={task.wikiUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-gold">Wiki <ExternalLink className="size-3" /></a>}</div></div></div></div>; }
