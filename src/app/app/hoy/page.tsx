...
import { AlarmClock, BrainCircuit, Check, Clock3, Flame, Gauge, Route, Sparkles, Target, TimerReset, WandSparkles, Zap } from "lucide-react";
      ...
      <section className="relative overflow-hidden rounded-3xl border border-gold/25 bg-gradient-to-br from-[#1b3022] via-[#121b16] to-[#12130f] p-6 sm:p-8">
        ...
        <div className="relative grid gap-8 xl:grid-cols-[1fr_350px] xl:items-center">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-muted">CapeForge analizó los pendientes de <strong className="text-cream">{data.settings.targetLabel}</strong>, el tiempo disponible, tareas con espera y tu progreso reciente. Hoy está optimizando para {data.settings.focusLabel}.</p>
            <div className="mt-6 flex flex-wrap gap-2"><Badge tone="green"><Check className="mr-1 size-3" /> Ruta actualizada hoy</Badge><Badge tone="gold"><Target className="mr-1 size-3" /> {data.settings.targetLabel}</Badge><Badge tone="purple"><Clock3 className="mr-1 size-3" /> {data.settings.sessionMinutes} min por sesión</Badge><Badge><Route className="mr-1 size-3" /> {data.dailyPlan.length} pasos sugeridos</Badge></div>
          </div>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_330px]">
        <section>
          ...
          <div className="space-y-4">
            ...
            {!data.dailyPlan.length && <div className="rounded-2xl border border-line bg-surface/60 p-10 text-center"><Sparkles className="mx-auto size-8 text-gold" /><h3 className="mt-4 font-display text-2xl">No quedan tareas pendientes.</h3><p className="mt-2 text-sm text-muted">Completaste el objetivo “{data.settings.targetLabel}”.</p></div>}
          </div>
        </section>

        <aside className="space-y-5">
          <form
            key={`${data.settings.targetId}:${data.settings.sessionMinutes}:${data.settings.focus}`}
          ...
          >
            ...
            <div className="mt-5"><label className="field-label" htmlFor="target">Quiero completar</label><select className="select-field" id="target" name="target" defaultValue={data.settings.targetId}>{data.settings.targets.map((target) => <option key={target.id} value={target.id}>{target.label}</option>)}</select><p className="mt-1.5 text-[10px] leading-4 text-muted">La inteligencia solo analizará tareas de este objetivo.</p></div>
            <div className="mt-4"><label className="field-label" htmlFor="sessionMinutes">Tiempo por sesión</label><select className="select-field" id="sessionMinutes" name="sessionMinutes" defaultValue={data.settings.sessionMinutes}>{[15, 30, 45, 60, 90, 120].map((minutes) => <option key={minutes} value={minutes}>{minutes} minutos</option>)}</select></div>
          ...
          <div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[.045] p-5"><span className="eyebrow text-emerald-300">Cómo aprende</span><ul className="mt-4 space-y-3 text-xs leading-5 text-muted"><li className="flex gap-2"><Check className="mt-0.5 size-3.5 shrink-0 text-emerald-300" /> Primero limita el análisis a tu objetivo elegido.</li><li className="flex gap-2"><Check className="mt-0.5 size-3.5 shrink-0 text-emerald-300" /> Las tareas que fijas siempre suben de prioridad.</li><li className="flex gap-2"><Check className="mt-0.5 size-3.5 shrink-0 text-emerald-300" /> Lo pospuesto desaparece y vuelve automáticamente.</li><li className="flex gap-2"><Check className="mt-0.5 size-3.5 shrink-0 text-emerald-300" /> Tu briefing rota cada día para evitar estancarte.</li></ul></div>
        </aside>
      </div>

      {data.timeGated.length > 0 && <section className="mt-10"><div className="mb-4"><span className="eyebrow"><AlarmClock className="size-3.5" /> No dejes pasar el tiempo</span><h2 className="mt-2 font-display text-3xl">Rituales y esperas.</h2><p className="mt-2 text-sm text-muted">Dentro de “{data.settings.targetLabel}”, CapeForge detectó estas tareas como diarias, semanales o time-gated.</p></div><div className="grid gap-4 lg:grid-cols-2">{data.timeGated.slice(0, 4).map((task) => <SmartTaskCard key={task.id} task={task} />)}</div></section>}
...
function CompactTask({ task, icon: Icon }: { task: Awaited<ReturnType<typeof getSmartRecommendations>>["dailyPlan"][number]; icon: typeof Zap }) {
  return (
    <TaskDetailDialog task={task}>
      <TaskDetailTrigger className="group flex items-center gap-3 rounded-xl border border-line bg-surface/55 p-3.5 transition hover:border-gold/30 hover:bg-white/[.035]">
        <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-white/4 text-gold transition group-hover:bg-gold/10"><Icon className="size-4" /></span>
        <span className="min-w-0 flex-1">
          <strong className="block truncate text-sm transition group-hover:text-gold-light">{task.name}</strong>
          <span className="text-[11px] text-muted">~{task.minutes} min · {task.category ?? "General"}</span>
        </span>
        <Badge tone={task.isGrind ? "purple" : "green"}>{task.compTimeType ?? "Sugerida"}</Badge>
      </TaskDetailTrigger>
    </TaskDetailDialog>
  );
}
